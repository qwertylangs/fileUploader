...
;// CONCATENATED MODULE: ./src/modules/upload.js
const eventListeners = new Map();
EventTarget.prototype.addEventListener_ =
  EventTarget.prototype.addEventListener;

EventTarget.prototype.removeEventListener_ =
...
EventTarget.prototype.removeEventListener = function (eventName, fn) {
  let event = eventListeners.get(this);
  if (event) {
    let eventData = event[eventName];
    eventData = eventData.filter((evfn) => evfn !== fn);
  ...
  EventTarget.prototype.removeEventListener_.call(this, eventName, fn);
};

function hasListeners(node, eventName) {
  if (!eventListeners.has(node)) return null;
  const event = eventListeners.get(node);
...
function upload(selector, options = {}) {
  ...
  function changeHandler(e) {
    ...
    // добавить обработчик если он был удален
    if (!hasListeners(upload, "click")) {
  ...
  function uploadHandler() {
    preview
      ...
      .forEach((cross) => cross.remove());

    const previewInfo = preview.querySelectorAll(".preview-info");
    previewInfo.forEach((info) => clearPreview(info));

    onUpload(files, previewInfo);
    upload.removeEventListener("click", uploadHandler);
  }

  function clearPreview(info) {
    info.style.bottom = "0.5rem";
    info.innerHTML = `<div class="preview-info-progress"></div>`;
  }
...
const firebaseConfig = {
  apiKey: 'AIzaSyDfQc2WpT-yp5p3_jqpez8X7ruHvfgPkOU',
  authDomain: 'fe-upload-e7348.firebaseapp.com',
  projectId: 'fe-upload-e7348',
  storageBucket: 'fe-upload-e7348.appspot.com',
  messagingSenderId: '575154982746',
  appId: '1:575154982746:web:d30805c468d5c8d0862d63',
  measurementId: 'G-QNFQLBDPH9',
};
...
upload('#file', {
  ...
  onUpload(files, blocks) {
    files.forEach((file, i) => {
      uploadfiles(file, i, blocks);
    });
  },
});

function uploadfiles(file, i, blocks) {
